...
public enum DataCategory {
    ...
    public static DataCategory fromShortName(String shortName) {
        if (shortName == null || shortName.isBlank()) return null;
        for (DataCategory cat : values()) {
            if (cat.shortName.equalsIgnoreCase(shortName)) {
                return cat;
            }
        }
        throw new IllegalArgumentException("Unknown DataCategory shortName: " + shortName);
    }
